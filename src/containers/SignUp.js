    ...
    if (isOk) {
      try {
        const response = await Axios.post(
          'http://localhost:5000/api/v1/user/sign_up',
          {
            // const response = await Axios.post("https://leboncoinhl.herokuapp.com/api/user/v1/sign_up", {
            username: userName,
            email: email,
            password: password,
          }
        ...
        if (response.data) {
          // With token sent with json response
          const { token } = response.data
          Cookies.set('token', token)
          // With token set in a cookie from the back-end
    ...
    <Wrapper>
      <section className='section d-flex'>
        ...
        <article className='contact-form fl-col'>
          <form className='full-width fl-col-left' onSubmit={handleSubmit}>
            ...
            <button
              type='submit'
              className={isOk ? `btn buttonOk` : `btn button`}
            >
              Créer mon compte
            </button>
          </form>
